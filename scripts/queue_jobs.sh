...
pre_jid=$(sbatch --parsable --job-name=negcodeparrot_pre train.sbatch NegativeSOCodeParrot so pretrain_config lvwerra/codeparrot-small lm  "++training.num_train_epochs=1 ++task.dump_name='negative' ++training.dataloader_num_workers=16 ++num_proc=16 ++training.batch_size=4 ++training.learning_rate=1e-3")
echo "Submitted PreTrain (id=$pretrain_jid)"
train_jid=$(sbatch --parsable --dependency=afterok:$pre_jid --job-name=negcodeparrot1 \
  train.sbatch NegativeSOCodeParrot mbpp greene_config lvwerra/codeparrot-small lm \

#!/bin/bash
# Queue Slurm Jobs
pre_jid=$(sbatch --parsable --job-name=negcodeparrot_pre train.sbatch NegativeSOCodeParrot so pretrain_config lvwerra/codeparrot lm  "++training.max_steps=10000 ++training.logging_steps=50 ++training.save_steps=250 ++training.eval_steps=250 ++training.num_train_epochs=1 ++task.dump_name='negative'")

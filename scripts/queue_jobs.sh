...
train_jid=$(sbatch --parsable --job-name=negcodeparrot \
  train.sbatch NegativeSOCodeParrotSmall mbpp greene_config lvwerra/codeparrot-small lm \
  "is_checkpoint=True +model_path=best_models/SO.NegativeSOCodeParrotSmall ++training.batch_size=4 ++training.learning_rate=1e-3")

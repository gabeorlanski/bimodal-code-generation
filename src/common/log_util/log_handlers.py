...
__all__ = ["TQDMLoggingHandler", "CompactFileHandler"]
...
class TQDMLoggingHandler(logging.Handler):
    ...
    -function-to-tqdm-write-so-log_util-doesnt-interfere-wit
    """
...
class CompactFileHandler(RotatingFileHandler):
    ...
    def __init__(
        self, file: str, level: int = logging.NOTSET, fmt: logging.Formatter = None
    ):
        """
        Initialize the handler
        Args:
            file: file to write to
            level: The log_util level
            fmt: Logging formatter
        """
        # Clear file
        with open(file, "w", encoding="utf-8") as f:
            f.write("")
        super(CompactFileHandler, self).__init__(
            file, mode="w", encoding="utf-8", maxBytes=10485760, backupCount=3
        )
        self.setLevel(level)
        self.setFormatter(
            fmt if fmt is not None else logging.Formatter("%(levelname)8s: %(message)s")

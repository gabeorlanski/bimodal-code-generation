...
__all__ = ["setup_basic_loggers", "prepare_global_logging"]
...
def prepare_global_logging(
    ...
    if world_size == 1:
        ...
        log_file = os.path.join(serialization_dir, f"{log_name}_worker{rank}.log")
    ...
    verbose_format = logging.Formatter(fmt=verbose_format, datefmt="%Y-%m-%d %H:%M:%S")

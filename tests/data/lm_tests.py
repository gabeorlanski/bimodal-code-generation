...
class TestConstantLengthDataset:
    ...
    def test_iter(self, tokenizer, infinite):
        ...
        seq_length = 20
        expected_examples, rem = divmod(sum(map(lambda t: len(t) + 1, example_tokenized)),
                                        seq_length)
        expected_examples += rem > 0

        dataset = langauge_modeling.ConstantLengthDataset(
            tokenizer,
            [{'input_sequence': t} for t in example_data],
            infinite=infinite,

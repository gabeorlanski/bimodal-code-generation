...
def test_train_model(tmpdir, training_args, tiny_model_name, device_val):
    ...
    task_cfg = {
        "name": "mbpp",
        "paths": {"train": "MBPP/train.jsonl", "validation": "MBPP/validation.jsonl"},
    }
    ...
    task = MBPP(AutoTokenizer.from_pretrained(tiny_model_name))
    ...
    assert train_args.args == (tmpdir_path.joinpath(task_cfg["paths"]["train"]),)
    assert train_args.kwargs == {"set_format": "torch"}

    val_args = task.read_data.call_args_list[1]
    assert val_args.args == (tmpdir_path.joinpath(task_cfg["paths"]["validation"]),)
